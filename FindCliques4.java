...
class FindCliques4 implements Runnable{
    ...
    private void Cliques4(){
	    ...
	    if(!this.parameters.Clique_4.containsKey(current))continue;
            ...
            for(int i=0;i<arr.size();i++){
                ...
                for(int y=i;y<arr.size();y++){
                    ...
                    if(tmp != null) {
                        ...
                        Clique4 clique = new Clique4(t1, t2, new Edge(arr.get(y).vertex, tmp.vertex, tmp.weight), t1.weight+t2.weight+tmp.weight-current.weight);
                        C4Set.add(clique);
                    }
                }
            }
        ...
        for(int i=0;i<clique_array.length;i++){
            ...
            else if(this.parameters.C4.peek().weight < clique_array[i].weight && !this.parameters.C4Set.contains(clique_array[i])){
                this.parameters.C4Set.remove(this.parameters.C4.poll());
                this.parameters.C4.put(clique_array[i]);
                this.parameters.C4Set.add(clique_array[i]);
            }
            else if(this.parameters.C4.peek().weight >= clique_array[i].weight && this.parameters.C4.size() >= this.parameters.Size) break;
        }
