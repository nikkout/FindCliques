...
import baseline.BaselineWithMatrix;
...
@Slf4j
public class TestBaselineWithMatrix extends Common {

	//mvn -Dtest=TestBaselineWithMatrix#testBaseline100 test
	@Test
	public void testBaseline100() {
		...
		BaselineWithMatrix bl = new BaselineWithMatrix(graph, 100, 1.5);
		...
		assertTrue("Number of triangles found is "+topKT.size(), topKT.size()==100);
		for (int i = 99; i >= 0; i--) {
	...
	public void testBaseline1000() {
		...
		Graph graph = rg.getGraph();
		graph.toFileW(new File("email-Eu-coreW.txt"));
		BaselineWithMatrix bl = new BaselineWithMatrix(graph, 1000, 1.5);
		ArrayList<Triangle> triangles = time.executeBruteForce(this::fullEnum, graph.getAdjacencyMatrixW(),
				"Brute Force");
		PriorityQueue<Triangle> topKT = time.executeAlgorithm(bl::findTriangles, "Algorithm");
		for (int i = 999; i >= 0; i--) {
